...
export function populateMermaidNodes(flowChart: FlowChart, direction: string) {
  ...
  for (const layer of grid) {
    for (const node of layer) {
      if (flowChart.nodeList[node.id]) {
        nodes[node.id] = createNode(node.id, 100, 100, x, y, [], flowChart.nodeList[node.id].type);
      }
      x += 200;
    }
    x = 0;
  ...
  for (const nodeId in flowChart.nodeList) {
    nodes[nodeId].inputNodes.update((parentNodes) => {
      for (const parent of flowChart.nodeList[nodeId].parents) {
        parentNodes.add(nodes[parent.node.id]);
      }
      return parentNodes;
    });
    ...
    for (const child of flowChart.nodeList[nodeId].children) {
      if (child.node.depth - 1 > flowChart.nodeList[nodeId].depth) {
        ...
        for (let i = startLayer; i <= endLayer; i++) {
          const ghostNode: LayerNode = {id:`GHOST_${child.node.id}_${ghostNodeIncrement}`, children: [], parents: [], layer: i};
          ghostNodeCollection.push(ghostNode);
          ghostNodeIncrement++;
        }
        for (const parent of child.node.parents) ghostNodeCollection[0].parents.push(parent.node.id);
        ghostNodeCollection[ghostNodeCollection.length - 1].children.push(child.node.id);
        for (let i = 0; i < ghostNodeCollection.length - 1; i++) ghostNodeCollection[i].children.push(`GHOST_${child.node.id}_${i + 1}`);
        for (let i = endLayer; i >= startLayer; i--) if (ghostNodeCollection.length > 0) {
...
function balanceLayers(layerTracker: LayerTracker) {
  let longestLayer = 0;
  for (const layer in layerTracker) longestLayer = Math.max(layerTracker[layer].length, longestLayer);
  for (const layer in layerTracker) {
    if (layerTracker[layer].length < longestLayer) {
      const balanceArray: Array<LayerNode> = Array.from({length : longestLayer - layerTracker[layer].length}, () => {return {id: 'NULL_NODE', children: [], parents: [], layer: parseInt(layer)}});
...
function findBestSwapIndex(layerTracker: LayerTracker, node: LayerNode, initialIndex: number): number | null {
  ...
  for (let i = 0; i < layerTracker[layer].length; i++) {
    ...
    if (layerTracker[layer - 1]) {
      for (let j = i + 1; j < layerTracker[layer - 1].length; j++) {
        if (currentNode.parents.includes(layerTracker[layer - 1][j].id)) parentsAheadIndices.push(j);
      }
    }
    if (layerTracker[layer + 1]) {
      for (let j = i + 1; j < layerTracker[layer + 1].length; j++) {
        if (currentNode.children.includes(layerTracker[layer + 1][j].id)) childrenAheadIndices.push(j);
      }
    }
...
function makeGridSquare(grid: Array<Array<LayerNode>>) {
  ...
  while (grid.length < largestSubArray) grid.push(Array.from({length: largestSubArray}, () => {return {id: 'NULL_NODE', children: [], parents: [], layer: grid.length}}));
  for (const [i, array] of grid.entries()) {
    if (array.length < largestSubArray) {
      while (array.length < largestSubArray) array.push({id: 'NULL_NODE', children: [], parents: [], layer: i});
    }
  }
}
