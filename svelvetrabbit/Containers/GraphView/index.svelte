    ...
    <div class={`Node Node-${key}`}>
      {#each $nodesStore as node}
        {#if node.image && !node.data.label}
          <ImageNode {node} {key} {derivedEdges} {nodesStore}/>
          <!-- If node has html property:  -->
        {:else if node.data.html}
          <Node {node} {key} {derivedEdges} {nodesStore}>{@html node.data.html}</Node> <!-- Directly render HTML inside of Node Component  -->
        <!-- {:else if node.data.component}
          <Node {node} {key} {derivedEdges} {nodesStore}><{node.data.component} /></Node>   -->
        {:else}
          <Node {node} {key} {derivedEdges} {nodesStore}>{node.data.label}</Node>
        {/if}
      {/each}
    </div>
  </div>
