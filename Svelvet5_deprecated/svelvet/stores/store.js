          ...
          n.forEach((child) => {
            if (correctNode.childNodes.includes(child.id)) {
              child.position.x = Math.min(
                Math.max(child.position.x + e.movementX / scale, 1),
                bound.x - 50
              );
              child.position.y = Math.min(
                Math.max(child.position.y + e.movementY / scale, 1),
                bound.y - 50
              );
            }
          });
          correctNode.position.x = Math.min(
            Math.max(correctNode.position.x + e.movementX / scale, 1),
            bound.x - 50
          );
          correctNode.position.y = Math.min(
            Math.max(correctNode.position.y + e.movementY / scale, 1),
            bound.y - 50
          );
        } else {
  ...
  const deleteNode = (e, $nodeIdSelected) => {
    ...
    if (answer) {
      ...
      coreSvelvetStore.edgesStore.update((e) => {
        return e.filter(
          (edge) =>
            edge.source !== $nodeIdSelected && edge.target !== $nodeIdSelected
        );
      });
    }
  };
  ...
  const derivedEdges = derived(
    ...
    ([$nodesStore, $edgesStore]) => {
      $edgesStore.forEach((edge) => {
        ...
        if (!$nodesStore.some((node) => node.id === edge.target))
          targetNode = null;
        // Do the same for the source
        if (!$nodesStore.some((node) => node.id === edge.source))
          sourceNode = null;
